...
describe("calculateCanvasPreviewTransform", () => {
    test("把已栅格视口映射到实时缩放视口", () => {
        expect(calculateCanvasPreviewTransform({ x: 80, y: 25, k: 1.5 }, { x: 100, y: 50, k: 1 })).toEqual({ ratio: 1.5, x: -70, y: -50 });
    });

    test("仅平移时保持单位缩放", () => {
        expect(calculateCanvasPreviewTransform({ x: 135, y: 70, k: 1.25 }, { x: 100, y: 50, k: 1.25 })).toEqual({ ratio: 1, x: 35, y: 20 });
    });
});
