                ...
                <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    <Field label="输出分辨率">
                        <Select
                            ...
                            onChange={(resolutions) => update({ resolutions, defaultResolution: resolutions.includes(profile.defaultResolution) ? profile.defaultResolution : resolutions[0] || "720p" })}
                        />
                    </Field>
                    <Field label="默认分辨率">
                        <Select
...
function NumberField({ label, value, min, disabled, onChange }: { label: string; value?: number; min: number; disabled: boolean; onChange: (value: number | null) => void }) {
    return (
        <Field label={label}>
            <InputNumber className="w-full" disabled={disabled} min={min} precision={0} value={value} onChange={onChange} />
        </Field>
    );
}
    ...
    return (
        <div className="flex items-center justify-between rounded-md border border-border/60 px-2.5 py-2">
            <div>
                ...
                <div className="text-[var(--fs-tiny)] text-foreground/45">支持该参数</div>
            </div>
