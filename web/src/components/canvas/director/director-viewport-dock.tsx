...
export function DirectorViewportDock({ transformMode, renderMode, onTransformModeChange, onRenderModeChange, onAddActor, onAddBox, onAddLight, onAddCamera, onAlignCamera }: DirectorViewportDockProps) {
    return (
        <nav className="director-viewport-dock" aria-label="导演台视口工具">
            ...
            <DockDivider />
            <DockButton label="构图预览" active={renderMode === "beauty"} onClick={() => onRenderModeChange("beauty")}>
                <Camera />
            </DockButton>
            <DockButton label="彩色白膜" active={renderMode === "clay"} onClick={() => onRenderModeChange("clay")}>
                <Palette />
            </DockButton>
            <DockButton label="骨骼视图" active={renderMode === "pose"} onClick={() => onRenderModeChange("pose")}>
                <Bone />
            </DockButton>
        </nav>
    );
