    ...
    return (
        <div data-canvas-no-zoom className="fixed inset-0 z-[var(--z-toast)] flex min-h-0 flex-col overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <header className="flex h-12 shrink-0 items-center gap-2 border-b px-2" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border }}>
                <IconButton label="关闭导演台" onClick={onClose}>
                    <X className="size-4" />
                </IconButton>
            ...
            <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)_292px] max-lg:grid-cols-[180px_minmax(0,1fr)]">
                <aside className="thin-scrollbar min-h-0 overflow-y-auto border-r" style={{ background: theme.node.panel, borderColor: theme.toolbar.border }}>
                    <PanelTitle
                        title="场景对象"
                        action={
                            <IconButton label="添加立方体" onClick={() => addPrimitive("box", "立方体")}>
                    ...
                    <PanelTitle
                        title="摄影机"
                        action={
                            <IconButton label="添加摄影机" onClick={addCamera}>
                                <Plus className="size-3.5" />
                            </IconButton>
                        }
                    />
                    ...
                    <PanelTitle
                        title="灯光"
                        action={
                            <IconButton label="添加灯光" onClick={addLight}>
                                <Plus className="size-3.5" />
                            </IconButton>
                        }
                    />
                    <div className="px-2 pb-2">
                        {draft.lights.map((light) => (
                            <SceneRow key={light.id} active={selectedLightId === light.id} icon={<Lightbulb />} label={light.name} onClick={() => setSelectedLightId(light.id)} />
                        ))}
                    </div>
                        ...
                        <>
                            ...
                            <div className="px-2 pb-3">
                                {imageNodes.slice(0, 20).map((node) => (
                                    <SceneRow key={node.id} icon={<ImageIcon />} label={node.title} onClick={() => addBillboard(node)} />
                                ))}
                            </div>
                        </>
                    ) : null}
                ...
                <aside className="thin-scrollbar min-h-0 overflow-y-auto border-l max-lg:hidden" style={{ background: theme.node.panel, borderColor: theme.toolbar.border }}>
                    {selectedObject ? (
                        <ObjectInspector
                            ...
                            onDelete={() => {
                                commit((current) => ({ ...current, objects: current.objects.filter((item) => item.id !== selectedObject.id) }));
                                setSelectedObjectId(null);
                            }}
                        />
                    ) : selectedLight ? (
                        <LightInspector
                            light={selectedLight}
                            onUpdate={(patch) => updateLight(selectedLight.id, patch)}
                            onDelete={() => {
                                commit((current) => ({ ...current, lights: current.lights.filter((item) => item.id !== selectedLight.id) }));
                                setSelectedLightId(null);
                            }}
                        />
                    ) : (
            ...
            {object.kind === "actor" || object.primitive === "character" || motionClips.length ? (
                <>
                    ...
                    {motionClips.length ? (
                        ...
                        <div className="text-[var(--fs-tiny)] opacity-50">模型加载后会显示可用动作 Clip</div>
                    )}
                    {mappedBones.length ? (
                        <Field label="骨骼控制">
...
function SceneRow({ active, icon, label, onClick }: { active?: boolean; icon: ReactElement; label: string; onClick: () => void }) {
    return (
        <button type="button" className={`flex h-8 w-full items-center gap-2 px-2 text-left text-xs transition ${active ? "bg-black/10 dark:bg-white/10" : "hover:bg-black/5 dark:hover:bg-white/5"}`} onClick={onClick}>
