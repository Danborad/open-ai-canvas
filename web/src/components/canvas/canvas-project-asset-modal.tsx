...
export function CanvasProjectAssetModal({
    ...
    return (
        <Modal open={open} title={null} footer={null} destroyOnHidden onCancel={onClose} width="min(920px, calc(100vw - 24px))" styles={{ container: { padding: 0, overflow: "hidden" }, body: { padding: 0 } }}>
            <div className="flex h-[min(620px,calc(100vh-80px))] min-h-[440px] flex-col overflow-hidden">
                <header className="flex h-12 shrink-0 items-center justify-between border-b border-border py-0 pl-4 pr-12">
                    ...
                    <span className="text-[var(--fs-label)] text-foreground/42">已选 {selectedIds.size} 项</span>
                </header>
                    ...
                    <nav className="thin-scrollbar flex min-w-0 gap-1 overflow-x-auto border-b border-border p-2 sm:block sm:overflow-y-auto sm:border-b-0 sm:border-r" aria-label="项目资产分类">
                        {categories.map((item) => (
                            ...
                            >
                                ...
                                <span className="min-w-5 rounded bg-foreground/[.05] px-1 text-center text-[var(--fs-tiny)] tabular-nums">{item === "all" ? items.length : items.filter((asset) => asset.category === item).length}</span>
                            </button>
                        ))}
                    </nav>
                ...
                <footer className="flex h-12 shrink-0 items-center justify-between border-t border-border px-3">
                    <span className="text-[var(--fs-tiny)] text-foreground/42">角色引用会在生成时解析当前角色版本</span>
    ...
    return (
        ...
        >
            <div className="relative aspect-[4/3] overflow-hidden bg-foreground/[.04]">
                {cover ? (
                    <img src={cover} alt={title} loading="lazy" decoding="async" className="h-full w-full object-contain p-1" />
                ) : (
                    <AssetMediaPreview
                        asset={media}
                        alt={title}
                ...
                <span className="absolute bottom-1.5 left-1.5 rounded bg-black/55 px-1.5 py-0.5 text-[var(--fs-micro)] text-white">{label}</span>
            </div>
            <div className="px-2 py-1.5">
                <div className="truncate text-[var(--fs-label)] font-medium">{title}</div>
                {character ? (
                    <div className="mt-0.5 truncate text-[var(--fs-micro)] text-foreground/42">
