...
function ProtocolBrandMark({ protocol, compact = false }: { protocol: ModelProtocolDefinition; compact?: boolean }) {
    ...
    if (protocol.value.startsWith("newapi-channel-"))
        return (
            <span className={cn("grid shrink-0 place-items-center rounded-md bg-muted text-foreground/65", iconSize)}>
    ...
    const brand =
        protocol.value === "gemini-veo" || protocol.value === "flow2api-image" || protocol.value === "flow2api-video"
            ? "gemini"
            : protocol.value === "grok-image" || protocol.value === "grok2api-image" || protocol.value === "grok2api-video" || protocol.value === "grok2api-new-image" || protocol.value === "grok2api-new-video" || protocol.value === "xai-video"
              ? "grok"
              : "openai";
