...
export default function CreatePage() {
    ...
    const updateAssistant = useCallback(
        (id: string, updater: (item: CreationMessage) => CreationMessage) => {
        ...
        try {
            if (mode === "text") {
                ...
                const storedImages = await Promise.allSettled(
                    generatedImages.map(async ({ image, taskId, resultIndex }) => {
                        const uploaded = await persistCreationImageResult(image);
                        addCreationAssetOnce(
                            creationImageAsset({
                                title: expandedPrompt.slice(0, 24),
                                uploaded,
                                metadata: { source: "create-generation", conversationId: activeConversation.id, messageId: assistantMessage.id, taskId, taskIds: boundTaskIdList, resultIndex, prompt: expandedPrompt },
                            }),
                            { taskId, messageId: assistantMessage.id, resultIndex },
                        );
                        return uploaded.url;
                    }),
                );
                ...
                addCreationAssetOnce(
                    creationVideoAsset({
                        ...
                        metadata: { source: "create-generation", conversationId: activeConversation.id, messageId: assistantMessage.id, taskId, taskIds: Array.from(boundTaskIds), resultIndex: 0, prompt: expandedPrompt },
                    }),
                    { taskId, messageId: assistantMessage.id, resultIndex: 0 },
                );
        ...
        <>
            <div className="creation-home relative flex h-full min-h-0 flex-col overflow-hidden">
                <div className="creation-top-actions">
                    {!isEmpty ? (
                        <Tooltip title="新建创作">
                            <button type="button" aria-label="新建创作" className="creation-top-action" onClick={startNewConversation}>
                                <Plus />
                            </button>
                        </Tooltip>
                    ) : null}
                ...
                <main ref={threadScrollRef} onScroll={handleThreadScroll} className="creation-scrollbar flex h-full min-h-0 flex-col overflow-y-scroll overscroll-contain">
                    {isEmpty ? (
                        <section className="creation-empty-workspace">
                            <CreationEmptyArt />
...
function CreationAssetLibraryModal({
    ...
    return (
        <Modal open={open} footer={null} title={null} destroyOnHidden onCancel={onClose} width="min(980px, calc(100vw - 24px))" className="creation-asset-library-modal" styles={{ container: { padding: 0 }, body: { padding: 0 } }}>
            <div className="creation-library-shell">
                    ...
                    <div className="creation-library-grid-wrap">
                        <div className="creation-library-grid">
                            {visibleAssets.length ? (
                                visibleAssets.map((asset) => <CreationLibraryCard key={asset.id} asset={asset} selected={selected.has(asset.id)} disabled={mode !== "video" && asset.kind === "video"} onToggle={() => toggle(asset)} />)
                            ) : (
                ...
                <footer className="creation-library-footer">
                    <button type="button" className="creation-library-upload" onClick={onUpload}>
                        <Upload />
                        <span>
                            <strong>找不到素材，手动上传</strong>
                            <small>支持图片{mode === "video" ? "和视频" : ""}，可一次选择多个文件</small>
                        </span>
                    </button>
                    <div className="creation-library-actions">
                        ...
                        <button type="button" className="is-primary" disabled={!selectedAssets.length} onClick={() => onConfirm(selectedAssets)}>
                            <Check />
    ...
    return (
        <button type="button" className={`creation-library-card${selected ? " is-selected" : ""}${disabled ? " is-disabled" : ""}`} onClick={onToggle} disabled={disabled} aria-pressed={selected}>
            <div className="creation-library-card-media">
                <AssetMediaPreview asset={asset} alt={asset.title} fallback={<div className="creation-library-card-fallback">{isVideo ? <Film /> : <ImageIcon />}</div>} />
                <span className="creation-library-card-check">
                    <Check />
                </span>
                <span className="creation-library-card-kind">{isVideo ? "视频" : "图片"}</span>
                {disabled ? <span className="creation-library-card-lock">视频仅支持视频创作</span> : null}
            </div>
...
function CreationMessageView({ item, modelName, onRetryFailure, onCreateVariant }: { item: CreationMessage; modelName: string; onRetryFailure: () => void; onCreateVariant: () => void }) {
    ...
    return (
        <article className="creation-assistant-message">
            ...
            {mode === "text" ? (
                <div className="creation-message-content">{item.content ? <ReactMarkdown>{item.content}</ReactMarkdown> : <span>正在生成…</span>}</div>
            ) : (
...
function CreationUserMessage({ item }: { item: CreationMessage }) {
    ...
    return (
        <div className="creation-user-message">
            <div>{displayCreationPrompt(item.content, item.references || [])}</div>
                        ...
                        return (
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    setPreviewType(video ? "video" : "image");
                                    setPreviewUrl(video ? attachment.url : url);
                                }}
                            ...
                            >
                                {isVideoAttachment(attachment) ? (
                                    ...
                                    <img src={url} alt={attachment.name} width={44} height={44} loading="lazy" />
                                )}
...
function MediaResult({ item, onRetryFailure, onCreateVariant }: { item: CreationMessage; onRetryFailure: () => void; onCreateVariant: () => void }) {
    ...
    if (item.status === "pending")
        return (
            <div className="creation-media-pending">
                <Spin size="small" />
                正在生成{item.mode === "video" ? "视频" : "图片"}…
            </div>
        );
            ...
            <div className="creation-chat-writing-surface">
                ...
                <div className="creation-chat-editor">
                    <CanvasResourceMentionTextarea
                        ...
                        maxLength={props.mode === "video" ? props.videoProfile.references.promptMaxChars : props.mode === "image" ? props.imageProfile.references.promptMaxChars : undefined}
                        ...
                        placeholder={props.variant === "empty" ? emptyPlaceholder : placeholder}
                        ...
                        <div className="creation-chat-attachment-strip">
                            {props.attachments.map((item) => (
                                <div key={item.id} className="creation-chat-attachment">
                                    ...
                                    <button type="button" onClick={() => props.onRemoveAttachment(item.id)} aria-label={`移除 ${item.name}`}>
                                        <X />
                                    </button>
                                </div>
                            ))}
            ...
            {props.mode === "video" ? (
                ...
                <>
                    {props.imageProfile.quality.supported ? (
                        <SettingSection title="图片质量" value={qualityLabel}>
                            <div className="creation-choice-grid is-quality">
                                {qualityOptions
                                    .filter((option) => props.imageProfile.quality.values.includes(option.value))
                                    .map((option) => (
...
const creationEmptyArtLibrary = [
    "black-white-noir.jpg",
    "chinese-2d.jpg",
    "clay-stop-motion.jpg",
    "comic-pop.jpg",
    "cyberpunk-neon.jpg",
    "fantasy-3d.jpg",
    "future-tech.jpg",
    "ink-narrative.jpg",
    "nature-healing.jpg",
    "period-live-action.jpg",
    "real-life.jpg",
    "retro-hong-kong.jpg",
    "space-opera.jpg",
    "storybook-fantasy.jpg",
    "surreal-dream.jpg",
    "suspense-noir.jpg",
    "three-d-cartoon.jpg",
    "urban-live-action.jpg",
...
function CreationEmptyArt() {
    ...
    return (
        <div className="creation-empty-art" aria-label="随机轮播的创作风格参考图">
            <div className="creation-empty-art-frame is-back">
                <img key={imageAt(0)} src={imageAt(0)} alt="" />
            </div>
            <div className="creation-empty-art-frame is-main">
                <img key={imageAt(1)} src={imageAt(1)} alt="" />
                <span>你的下一帧，从这里开始</span>
            </div>
            <div className="creation-empty-art-frame is-front">
                <img key={imageAt(2)} src={imageAt(2)} alt="" />
            </div>
            <div className="creation-empty-art-caption">
...
function CreationIntro({ mode }: { mode: CreationMode }) {
    ...
    return (
        <header className="creation-chat-intro" aria-live="polite">
            <span className="creation-intro-signal" aria-hidden="true" />
            <h1>
                {copy[0]}
                <span className="creation-intro-emphasis">
                    <span className="is-pink">{copy[1].slice(0, 1)}</span>
                    <span className="is-blue">{copy[1].slice(1)}</span>
                </span>
                {copy[2]}
            </h1>
            <p>{copy[3]}</p>
            ...
            try {
                ...
                if (images.length) {
                    const storedImages = await Promise.all(
                        images.map(async (image, resultIndex) => {
                            const uploaded = await persistCreationImageResult(image);
                            addCreationAssetOnce(
                                creationImageAsset({
