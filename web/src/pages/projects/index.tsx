...
export default function ProjectsPage() {
    ...
    return (
        <WorkspacePage className="library-page" grid>
            <PageHeader icon="projects" title="短剧创作" description="按时间浏览故事项目，继续最近的章节、画布与镜头。" meta={<span className="text-xs text-foreground/45">{rows.length} 个</span>} actions={null} />
            <ListToolbar
            ...
            {!query.isLoading && !query.isError && (rows.length || canCreateProject) ? (
                <CollectionGrid className="library-grid project-library-grid">
                    {canCreateProject ? (
                        <button type="button" className="library-create-card" onClick={() => setCreateOpen(true)}>
