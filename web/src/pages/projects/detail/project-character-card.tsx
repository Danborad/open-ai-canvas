        ...
        <AssetLibraryCard className="project-character-card">
            <AssetLibraryCardMedia className="relative aspect-[3/2] overflow-hidden bg-foreground/[.045]">
                <button type="button" className="project-asset-media-button" onClick={onOpen} aria-label={`查看角色卡：${asset.title}`}>
                    {cover ? (
                        <img src={resourceFileUrl(cover.resourceId)} alt={asset.title} loading="lazy" decoding="async" className="h-full w-full object-contain p-1" />
                    ) : (
            ...
            <div className="p-3">
                <div className="flex items-start justify-between gap-3">
                    ...
                    <Button type="text" size="small" className="!h-7 !px-1.5" icon={<Pencil className="size-3.5" />} onClick={onEdit} aria-label={`编辑 ${asset.title}`} />
                </div>
                ...
                <div className="mt-2 grid gap-1.5">
                    <StatusLine
                        ...
                        label={imageStatus}
                        action={character?.visualStatus === "ready" ? "更换" : "初始化"}
                ...
                <div className="mt-3 flex min-w-0 gap-2 border-t border-border/60 pt-2">
                    <Button size="small" className="min-w-0 flex-1" icon={<Sparkles className="size-3.5" />} loading={generating} disabled={removing} onClick={onGenerate}>
                        {character?.visualStatus === "missing" ? "初始化三视图" : "重新生成三视图"}
                    </Button>
                    <Popconfirm title="移出项目角色？" description="已有画布或镜头引用时将无法移出。" okText="移出" cancelText="取消" onConfirm={onRemove}>
                        <Button type="text" danger size="small" loading={removing} disabled={generating} icon={<Trash2 className="size-3.5" />} aria-label={`移出 ${asset.title}`} />
                    </Popconfirm>
                </div>
            </div>
        </AssetLibraryCard>
