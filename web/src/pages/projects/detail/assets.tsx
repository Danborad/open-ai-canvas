...
export default function ProjectAssetsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const addMutation = useMutation({
        mutationFn: () => linkProjectAsset(detail.project.id, { assetId, category: assetCategory }),
        onSuccess: ({ asset }) => {
    ...
    const bindVoiceMutation = useMutation({
        mutationFn: () => (voiceAsset ? bindProjectCharacterVoice(detail.project.id, voiceAsset.id, { voiceProfileId, instructions: voiceInstructions }) : Promise.reject(new Error("未选择角色"))),
        onSuccess: (result) => {
            syncPersonalCharacterProjection(result.asset);
            setVoiceAsset(null);
            done("声音素材已绑定到新角色版本");
    ...
    const openImages = (asset: ProjectAsset) => {
        setImageAsset(asset);
        setImageSelection("");
    };
    ...
    return (
        <div>
                    ...
                    <div className="min-w-0">
                        ...
                        <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-[var(--fs-label)] text-foreground/48">
                            ...
                            {pendingCandidates.length ? (
                                <span className="inline-flex items-center gap-1.5 text-[var(--workspace-accent)]">
                                    <Sparkles className="size-3.5" />
                                    {pendingCandidates.length} 个待确认
                                </span>
                            ) : null}
                        </div>
                    </div>
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-[156px_minmax(0,1fr)]">
                <nav className="space-y-0.5 border-r border-border/70 pr-2" aria-label="资产分类">
                    {categoryCounts.map((item) => (
                        <button
                            key={item.value}
                            type="button"
                            onClick={() => setCategory(item.value)}
                            className={`flex h-11 w-full items-center justify-between rounded-md px-2 text-left text-xs ${category === item.value ? "bg-foreground/[.08] font-medium" : "text-foreground/55 hover:bg-foreground/[.04]"}`}
                        >
                ...
                <div className="min-w-0">
                    ...
                    <div className="mb-2 flex items-center justify-between text-xs text-foreground/45">
                        <span>{category === "all" ? "全部资产" : categoryLabel(category)}</span>
                        <span>{visibleAssets.length} 项已确认</span>
                    </div>
                            ...
                            {visibleAssets.map((asset) =>
                                asset.category === "character" ? (
                                    <ProjectCharacterCard
                                        ...
                                        removing={unlinkMutation.isPending && unlinkMutation.variables === asset.id}
                    ...
                    ) : (category === "all" || category === "character") && pendingCandidates.length ? null : (
                        <WorkspaceState icon="assets" compact title="这个分类还没有资产" description={category === "character" ? "新建角色，或先从剧情章节提取角色信息。" : "可从个人素材库引用，或切换到其他分类查看。"} />
                    )}
                </div>
            </div>

            <Modal title="引用个人素材" open={addOpen} okText="加入项目" cancelText="取消" okButtonProps={{ disabled: !assetId, loading: addMutation.isPending }} onCancel={() => setAddOpen(false)} onOk={() => addMutation.mutate()} width={480}>
                <div className="grid gap-3">
                    <Select
                        showSearch
                        optionFilterProp="label"
                        value={assetId || undefined}
                        placeholder="选择未加入项目的素材"
                        options={availableAssets.map((asset) => ({ label: `${asset.title} · ${mediaLabel(asset.kind)}`, value: asset.id }))}
                        onChange={(value) => {
                            setAssetId(value);
                            const next = personalAssets.find((asset) => asset.id === value);
                            setAssetCategory(next?.kind === "entity" ? "character" : next?.category || "other");
                        }}
                    />
                    <Select
                        value={assetCategory}
                        disabled={selectedPersonalAsset?.kind === "entity"}
                        options={
                            selectedPersonalAsset?.kind === "entity"
            ...
            />
            <Modal
                title={`绑定单张三视图 · ${imageAsset?.title || ""}`}
                open={Boolean(imageAsset)}
                okText="绑定并生成新版本"
                cancelText="取消"
                width={720}
                okButtonProps={{ loading: bindImagesMutation.isPending, disabled: !imageSelection }}
                onCancel={() => setImageAsset(null)}
                onOk={() => bindImagesMutation.mutate()}
            >
                {imageAssets.length ? (
                    <div className="grid max-h-[420px] gap-3 overflow-y-auto sm:grid-cols-3">
                        {imageAssets.map((asset) => (
                            <button
                                type="button"
                                key={asset.id}
                                onClick={() => setImageSelection(asset.id)}
                                className={`overflow-hidden rounded-lg border text-left transition ${imageSelection === asset.id ? "border-[var(--workspace-accent)] ring-2 ring-[var(--workspace-accent)]/20" : "border-border/70 hover:border-foreground/35"}`}
                            >
                                <div className="relative aspect-[4/3] bg-foreground/[.045]">
                                    <img src={asset.data.dataUrl} alt={asset.title} loading="lazy" decoding="async" className="h-full w-full object-cover" />
                                    {imageSelection === asset.id ? (
                                        <span className="absolute right-2 top-2 grid size-6 place-items-center rounded-full bg-[var(--workspace-accent)] text-white">
                                            <Check className="size-4" />
                                        </span>
                                    ) : null}
                                </div>
                                <div className="truncate px-2 py-2 text-[var(--fs-label)] font-medium">{asset.title}</div>
                            </button>
                        ))}
                    </div>
                ) : (
                    <WorkspaceState icon="assets" compact title="个人图片素材为空" description="需要一张包含正面、侧面、背面的角色设定图。" />
                )}
            </Modal>
            <Modal
                title={`选择声音素材 · ${voiceAsset?.title || ""}`}
...
function CharacterEditorModal({ open, editing, form, loading, onClose, onSave }: { open: boolean; editing: boolean; form: FormInstance<CharacterForm>; loading: boolean; onClose: () => void; onSave: () => void }) {
    ...
    return (
        <Modal title={null} open={open} forceRender width={860} okText={editing ? "保存角色设定" : "创建角色卡"} cancelText="取消" okButtonProps={{ loading }} onCancel={onClose} onOk={onSave} styles={{ body: { paddingTop: 0 } }}>
            <div className="mb-1 border-b border-border/70 pb-4">
                <div className="text-[var(--fs-label)] font-medium text-[var(--workspace-accent)]">角色设定</div>
        ...
        <AssetLibraryCard className="project-asset-library-card">
            ...
            <div className="p-2.5">
                ...
                <div className="mt-1 flex items-center gap-1.5 text-[var(--fs-tiny)] text-foreground/42">
                    ...
                    >
                        <button type="button" disabled={loading} className="inline-flex h-6 items-center gap-1 rounded px-1.5 text-[var(--fs-tiny)] text-foreground/50 hover:bg-foreground/[.055]">
                    ...
                    <Link2 className="ml-auto size-3.5 shrink-0 text-[var(--workspace-accent)]" />
                </div>
                <div className="mt-2 flex items-center justify-between gap-2 border-t border-border/60 pt-1.5">
                    ...
                    <div className="flex items-center">
                        <Button type="text" size="small" icon={<RefreshCw className="size-3.5" />} loading={loading} onClick={onVersion} />
                        <Popconfirm title="移出项目资产？" okText="移出" cancelText="取消" onConfirm={onRemove}>
                            <Button type="text" danger size="small" icon={<Trash2 className="size-3.5" />} loading={loading} />
                        </Popconfirm>
                    </div>
                </div>
            </div>
...
function ProjectAssetPreviewModal({ asset, personalAsset, onClose, onDownload, onReplaceImage }: { asset: ProjectAsset | null; personalAsset?: Asset; onClose: () => void; onDownload: () => void; onReplaceImage: () => void }) {
    ...
    return (
        <Modal
            className="library-modal project-asset-preview-modal"
            ...
            {asset ? (
                <div className="project-asset-preview-layout">
                    <div className={`project-asset-preview-stage ${previewClass}`}>
                        {asset.category === "character" ? (
                            characterCover ? (
                                <img src={resourceFileUrl(characterCover.resourceId)} alt={asset.title} className="project-asset-preview-media" />
                            ) : (
                            ...
                            <img src={personalAsset.data.dataUrl || personalAsset.coverUrl} alt={asset.title} className="project-asset-preview-media" />
