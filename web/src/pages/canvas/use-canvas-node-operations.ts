...
export function useCanvasNodeOperations({
    ...
    const duplicateNode = useCallback(
        (nodeId: string) => {
            ...
            const nextNodes = [
                ...nodesRef.current.map((node) =>
                    node.id === source.id && versionRootId && !node.metadata?.versionLabel ? { ...node, title: `${node.title} · A`, metadata: { ...node.metadata, versionOfNodeId: versionRootId, versionLabel: "A", versionPrimary: true } } : node,
                ),
                ...copiedNodes,
            ];
            commitNodes(nextNodes);
