...
export function AssistantPanelColumn({ width, closing, topInset, onWidthChange, children }: { width: number; closing: boolean; topInset: string; onWidthChange: (width: number) => void; children: (resizing: boolean) => ReactNode }) {
    ...
    return (
        ...
        >
            <div className="h-full w-full">
                {!closing ? <button type="button" className="absolute inset-y-0 left-0 z-[var(--node-z-overlay)] w-4 -translate-x-1/2 cursor-col-resize" onMouseDown={startResize} aria-label="调整右侧面板宽度" /> : null}
                {children(resizing)}
            </div>
        </div>
    );
