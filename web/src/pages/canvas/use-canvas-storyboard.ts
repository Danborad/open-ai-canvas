...
import { backendProviderConfig, buildGenerationConfig, generationTaskMetadata, resetGenerationTaskMetadata } from "@/lib/canvas/canvas-project-generation";
...
export function useCanvasStoryboard({ projectId, nodesRef, connectionsRef, setNodes, setConnections, setSelectedNodeIds, enqueueGenerationBatch }: UseCanvasStoryboardOptions) {
            ...
            rows.forEach((row, index) => {
                ...
                const imageNode = existing
                    ...
                    : createCanvasNode(
                          ...
                          { prompt, ...storyboardPromptTemplateMetadata(row, "image"), workflowKind: "shot", workflowTitle: `镜头 ${row.shotNumber} 分镜图`, shotIndex: row.shotNumber, status: NODE_STATUS_IDLE },
                      );
                if (!existing) {
    ...
    const createAndGenerateScriptVideos = useCallback(
        async (nodeId: string, rowIds?: string[]) => {
            ...
            const dedicatedFirstFrameConnections = new Set(targets.filter((target) => Boolean(target.row.imageNodeId)).map((target) => `${target.row.imageNodeId}:${target.videoNode.id}`));
...
function characterReferenceNodeIds(row: StoryboardRow, nodes: CanvasNodeData[]) {
    ...
    return nodes.filter((node) => node.metadata?.workflowKind === "character" && Boolean(node.metadata.characterAssetId) && assetIds.has(node.metadata.characterAssetId!)).map((node) => node.id);
}
