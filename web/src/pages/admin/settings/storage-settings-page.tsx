    ...
    return (
        <AdminPageFrame title="存储服务" description="配置新增资源的默认存储位置">
            <div className="space-y-4 pt-4">
                <div className="border-b border-border px-1 pb-4 text-foreground/75">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-xs leading-6 text-foreground/55">存储类型只影响之后新增的媒体资源，不迁移或改写历史文件。日常读取继续校验登录态，仅在模型必须通过 URL 拉取本地素材时生成短时签名链接。</p>
                        </div>
                    </div>
                </div>
                <SettingsSectionCard
                    ...
                    status={
                        <Space size={6}>
                            <Tag variant="filled" color={setting?.enabled ? "blue" : "default"}>
                                {setting?.enabled ? "阿里云 OSS" : "服务器本地"}
                            </Tag>
                            {setting?.enabled ? (
                                <Tag variant="filled" color={setting.hasAccessKeySecret ? "success" : "warning"}>
                            ...
                            {mode === "oss" ? (
                                ...
                                <>
                                    ...
                                    <div className="md:col-span-2 border-t border-border py-4 text-xs leading-6 text-foreground/60">
                                        ...
                                        <div className="mt-1">
                                            域名已反向代理到本服务时，可点击“使用当前地址”，例如 <code className="text-foreground">https://ddcat.pronhubcn.com</code>。
                                        </div>
                                        <div>
                                            直接开放服务器端口时，填写完整的公网 IP 和端口，例如 <code className="text-foreground">http://103.242.14.110:300</code>。
                                        </div>
                                        <div>
                                            地址末尾不要填写 <code className="text-foreground">/api</code>；保存前可在浏览器访问 <code className="text-foreground">&lt;服务器访问地址&gt;/api/health</code>，看到{" "}
                                            <code className="text-foreground">status: ok</code> 即表示入口正确。
                                        </div>
                                        <div className="mt-1 text-foreground/45">
                                            新增资源写入 <code>CANVAS_BACKEND_DATA_DIR/resources/</code>，部署时必须持久化该数据卷。供外部模型使用时，以上地址也必须能被模型服务访问，生产环境建议使用 HTTPS。
                                        </div>
                                    </div>
                                </>
                            )}
                ...
                <div className="grid border-y border-border text-xs text-foreground/55 sm:grid-cols-3 sm:divide-x sm:divide-border">
                    <Notice icon={mode === "oss" ? <Cloud className="size-3.5" /> : <HardDrive className="size-3.5" />} text={mode === "oss" ? "新资源写入阿里云 OSS" : "新资源写入服务器数据卷"} />
                    <Notice icon={<ShieldCheck className="size-3.5" />} text="历史资源位置保持不变" />
                    <Notice icon={<KeyRound className="size-3.5" />} text="外部链接仅在签名有效期内可用" />
                </div>
            </div>
        </AdminPageFrame>
    );
...
function formValues(setting?: AdminOSSSetting | null): OSSFormValues {
    return {
        mode: setting?.enabled ? "oss" : "local",
