...
export default function EmailSettingsPanel() {
    ...
    return (
        <SettingsSectionCard
            ...
            footer={
                <>
                    <span className="text-xs text-foreground/45">SMTP 密码使用服务端密钥加密，接口不回显明文。</span>
        ...
        >
            <Form form={form} layout="vertical" requiredMark={false} disabled={loading}>
                <div className="grid gap-x-5 px-5 pt-5 md:grid-cols-2">
                    <Form.Item name="enabled" label="启用注册验证邮件" valuePropName="checked" extra="公开注册开启后，普通邮箱注册必须完成验证码校验。">
                        <Switch />
                    </Form.Item>
                    ...
                    <Form.Item name="port" label="SMTP 端口" rules={[{ required: true, type: "number", min: 1, max: 65535, message: "请输入 1-65535 的 SMTP 端口" }]}>
                        <InputNumber className="w-full" min={1} max={65535} precision={0} placeholder="587" />
                    </Form.Item>
                    <Form.Item name="username" label="SMTP 用户名">
                        <Input autoComplete="off" placeholder="通常为完整邮箱地址" />
                    </Form.Item>
                    <Form.Item name="password" label={setting?.hasPassword ? `SMTP 密码（${configuredSecretText}）` : "SMTP 密码"}>
                        <Input.Password autoComplete="new-password" placeholder={setting?.hasPassword ? "留空保留原密码" : "SMTP 密码或授权码"} />
                    </Form.Item>
                    <Form.Item name="fromEmail" label="发件邮箱" rules={[{ type: "email", message: "请输入有效的发件邮箱" }]}>
