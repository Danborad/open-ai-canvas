...
export default function StoryboardPromptsPage() {
    ...
    const columns: ColumnsType<PromptTemplate> = [
        ...
        },
        {
            ...
            width: 100,
            render: (enabled) => (
                <Tag variant="filled" color={enabled ? "success" : "default"}>
                    {enabled ? "启用中" : "历史版"}
                </Tag>
            ),
        },
    ...
    return (
        ...
        >
            <div className="border-b border-border pb-4 text-xs leading-5 text-foreground/60">
                <div className="flex items-center gap-1.5 font-medium text-foreground/80">
                    <ShieldCheck className="size-3.5" />
                    创作策略与执行契约分离
                </div>
                <p>可以调整导演方法、镜头语言和生成策略。JSON Schema、动态项目数据和服务端校验属于受保护契约，只读展示且不会被模板覆盖。</p>
            </div>
                ...
                <Form form={form} layout="vertical" requiredMark={false} className="flex min-h-full flex-col">
                    ...
                    <div className="grid min-h-0 flex-1 gap-0 lg:grid-cols-3">
                        ...
                        <aside className="min-h-0 p-4">
                            ...
                            />
                            <div className="mt-4 flex items-start gap-2 border-t border-border pt-4 text-xs leading-5 text-foreground/55">
                                <Info className="mt-0.5 size-3.5 shrink-0" />
                                每次调整都会创建新版本，启用中的版本不会被原地覆盖。
                            </div>
                        </aside>
                    </div>
                </Form>
